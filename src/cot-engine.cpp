...
void cot::Engine::update(const cot::math_t dt)
{
    ...
    for (std::size_t i = 0; i < this->vSystem.size(); i++)
    {
        for (std::size_t j = 0; j < i; j++)
        {
            ...
            std::pair<sf::Vector2f, sf::Vector2f> vForcePair = cot::Engine::force2(&this->vSystem[i].first, &this->vSystem[j].first);
...
std::pair<sf::Vector2f, sf::Vector2f> cot::Engine::force2(const cot::body_t *bodyA, const cot::body_t *bodyB)
{
    ...
    sf::Vector2f vDist;
    vDist.x = bodyA->position.x - bodyB->position.x;
    vDist.y = bodyA->position.y - bodyB->position.y;
    ...
    cot::math_t mForce = (mDist != 0.0f ? gConst * bodyA->mass * bodyB->mass * std::pow(mScale, 2.0f) / std::pow(mDist, 2.0f) : 0.0f);
